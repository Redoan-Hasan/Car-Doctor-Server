...
app.use(
  cors({
    origin: [
      "http://localhost:5173",
      "https://car-doctor-server-fawn-phi.vercel.app",
      "https://car-doctor-d9407.web.app",
      "https://car-doctor-d9407.firebaseapp.com",
    ],
    credentials: true,
  })
);
...
// verify Token through middleware 2.0
const verifyToken = async (req, res, next) => {
  const token = req?.cookies.token;
...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.qhz4s.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    const userOrderCollection = client
      .db("Car-doc-datas")
      .collection("car-doc-order");
